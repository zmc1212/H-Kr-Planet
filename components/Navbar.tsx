  ...
  return (
    <header className="fixed w-full z-50 top-0 left-0">
      ...
      <div className="bg-retro-orange text-black py-1 overflow-hidden border-b-2 border-black">
        <div className="whitespace-nowrap animate-scan flex space-x-8 text-xs font-bold tracking-widest font-mono">
           {[...Array(5)].map((_, i) => (
             <span key={i}>// 系统在线 // 连接至氢氪网络 // 延迟: 12ms // 内存状态: 正常 // 正在扫描扇区... // </span>
           ))}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
              ...
              <div className="ml-10 flex items-center space-x-1">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={`
                      px-4 py-2 text-sm font-bold tracking-wider border border-transparent hover:border-retro-orange hover:bg-retro-surface hover:text-retro-orange transition-all font-mono
            ...
            <div className="hidden md:flex items-center space-x-4 border-l-2 border-retro-gray pl-6">
               <div className="flex flex-col items-end">
                  <span className="text-[10px] text-retro-gray font-mono">SYS_STATUS</span>
                  <div className="flex items-center text-retro-green text-xs font-bold font-mono">
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-retro-bg border-b-2 border-retro-orange absolute w-full z-50 shadow-2xl">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navItems.map((item) => (
              <Link
                ...
                to={item.href}
                onClick={() => handleNavClick(item.href)}
                className="block px-3 py-4 text-base font-bold text-retro-white hover:bg-retro-orange hover:text-black border-l-4 border-transparent hover:border-black transition-all font-mono"
              >
                &gt; {item.label}
              </Link>
            ))}
